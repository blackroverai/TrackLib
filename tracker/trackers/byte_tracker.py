...
class ByteTracker(BaseTracker):
    ...
    def update(self, output_results, img, ori_img):
        ...
        if len(dets) > 0:
            if self.with_reid:
                detections = [Tracklet_w_reid(tlwh, s, cate, motion=self.motion, feat=feat, dt=1.0/self.frame_rate, kalman_kwargs=self.kalman_kwargs, group_map=self.group_map) for
                            (tlwh, s, cate, feat) in zip(dets, scores_keep, cates, features_keep)]
            else:
                detections = [Tracklet(tlwh, s, cate, motion=self.motion, dt=1.0/self.frame_rate, kalman_kwargs=self.kalman_kwargs, group_map=self.group_map) for
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [Tracklet(tlwh, s, cate, motion=self.motion, dt=1.0/self.frame_rate, kalman_kwargs=self.kalman_kwargs, group_map=self.group_map) for
